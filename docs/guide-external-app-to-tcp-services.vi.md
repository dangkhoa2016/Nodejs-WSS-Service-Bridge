...
> `Nodejs-WSS-Service-Bridge`. Một thiết lập điển hình: Redis chạy trên máy nhà/server
...
- **Server** (`Nodejs-WSS-Service-Bridge`) — relay luồng dữ liệu giữa client và các
...
| Server | Trung tâm relay (`Nodejs-WSS-Service-Bridge`) | Máy chủ công khai |
