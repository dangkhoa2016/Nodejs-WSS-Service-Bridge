...
> **Mới dùng dự án?** Hãy bắt đầu với [Use Case 2 — TCP Services](use-case-tcp.vi.md). Tài liệu này tập trung vào triển khai application/service nhiều host ở mức nâng cao.
...
```bash
curl -fsSL 'https://raw.githubusercontent.com/dangkhoa2016/Nodejs-WSS-Service-Bridge/<release-tag>/scripts/setup-service-host.sh' -o setup-service-host.sh
...
```bash
curl -fsSL 'https://raw.githubusercontent.com/dangkhoa2016/Nodejs-WSS-Service-Bridge/<release-tag>/scripts/setup-application-host.sh' -o setup-application-host.sh
