...
Tài liệu này hướng dẫn cách kích hoạt **tunnel TCP** của Nodejs-WSS-Service-Bridge để phơi bày các dịch vụ TCP (Redis, Postgres, MySQL...) đang chạy ở phía tunnel client (Google Colab, Kaggle, PC cục bộ) ra ngoài Internet thông qua máy chủ trung gian — ví dụ để một ứng dụng bên ngoài như **Rails** dùng Redis client kết nối vào Redis.
